...
None of this changes which engines answer. That is decided inside SearXNG, and the default set for the `general` category a text search asks for is small, so a thin result page is usually an engine problem rather than a resilience problem. `docs/configuration.md` covers replacing the SearXNG settings file when the container starts, which is how an operator widens the set or drops an engine that is blocked from their IP.
