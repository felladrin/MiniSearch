...
| A caller streams an oversized body to `/api/validate-access-key` | Refused with `413` mid-upload; the socket is closed rather than pooled, so the caller's next request still succeeds | `server/validateAccessKeyServerHook.socket.test.ts` |
