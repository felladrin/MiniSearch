...
| `textToSpeechEngine` | `"local" \| "system"` | `"local"` | Which engine reads answers aloud. `"system"` also overrides a local voice picked above, and stops the voice list from being fetched, so it never contacts a third-party host |
